...
class nppoiscvm : public npfixedcomp
{
public:
	Eigen::VectorXd weights;

	nppoiscvm(const Eigen::VectorXd &data_, const Eigen::VectorXd &weights_, const Eigen::VectorXd &mu0fixed_, const Eigen::VectorXd &pi0fixed_,
		const double &beta_, const Eigen::VectorXd &initpt_, const Eigen::VectorXd &initpr_, 
		const Eigen::VectorXd &gridpoints_) : npfixedcomp(data_, mu0fixed_, pi0fixed_, beta_, initpt_, initpr_, gridpoints_){
	...
	Eigen::VectorXd mapping(const Eigen::VectorXd &mu0, const Eigen::VectorXd &pi0) const{
		return pnppois_(this->data, mu0, pi0, this->beta);
	}
	...
	void gradfun(const double &mu, const Eigen::VectorXd &dens,
		...
		double scale = 1 - this->pi0fixed.sum();
		if (d0){
			ansd0 = (pnppois_(this->data, mu, scale, this->beta) - dens).dot(fullden) * 2;
		}
	}
	...
	void gradfunvec(const Eigen::VectorXd &mu, const Eigen::VectorXd &dens,
		...
		double scale = 1 - this->pi0fixed.sum();
		if (d0){
			ansd0 = (ppoisarray(this->data, mu, this->beta) * scale - dens.rowwise().replicate(mu.size())).transpose() * fullden * 2;
		}
	}

	void computeweights(const Eigen::VectorXd &mu0, Eigen::VectorXd &pi0, const Eigen::VectorXd &dens) const{
		pi0 = pnnlssum_(ppoisarray(this->data, mu0, this->beta).cwiseProduct(this->weights.cwiseSqrt().replicate(1, mu0.size())),
...
// [[Rcpp::export]]
Rcpp::List nppoiscvm_(const Eigen::VectorXd &data, const Eigen::VectorXd &weights, const Eigen::VectorXd &mu0fixed, const Eigen::VectorXd &pi0fixed,
	const double &beta, const Eigen::VectorXd &initpt, const Eigen::VectorXd &initpr, const Eigen::VectorXd &gridpoints,
	const double &tol = 1e-6, const int &maxit = 100, const int &verbose = 0){
	nppoiscvm f(data, weights, mu0fixed, pi0fixed, beta, initpt, initpr, gridpoints);
	f.computemixdist(tol, maxit, verbose);
	return f.result;
}

// [[Rcpp::export]]
Rcpp::List estpi0nppoiscvm_(const Eigen::VectorXd &data, const Eigen::VectorXd &weights,
	const double &beta, const double &val, const Eigen::VectorXd &initpt, const Eigen::VectorXd &initpr, const Eigen::VectorXd &gridpoints,
	const double &tol = 1e-6, const int &verbose = 0){
	nppoiscvm f(data, weights, Eigen::VectorXd::Zero(1), Eigen::VectorXd::Ones(1), beta, initpt, initpr, gridpoints);
	f.estpi0(val, tol, verbose);
	return f.result;
}
