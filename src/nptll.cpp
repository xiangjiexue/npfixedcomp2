...
public:
	...
	nptll(const Eigen::Matrix<Type, Eigen::Dynamic, 1> &data_, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &mu0fixed_, 
	...
	Type familydensity(const Type &x, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &mu0, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &pi0) const{
		return dnpt_(Eigen::Matrix<Type, Eigen::Dynamic, 1>::Constant(1, x), mu0, pi0, this->beta).coeff(0);
	}
};
...
public:
	...
	nptllw(const Eigen::Matrix<Type, Eigen::Dynamic, 1> &data_, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &weights_, 
	...
	void gradfun(const Type &mu, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &dens,
		...
		Eigen::Matrix<Type, Eigen::Dynamic, 1> fullden = (dens + this->precompute).cwiseInverse();
		Type scale = 1. - this->pi0fixed.sum();
		Eigen::Matrix<Type, Eigen::Dynamic, 1> temp = dnpdisct_(this->data, mu, scale, this->beta, this->h);
		if (d0){
	...
	Type familydensity(const Type &x, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &mu0, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &pi0) const{
		return dnpdisct_(Eigen::Matrix<Type, Eigen::Dynamic, 1>::Constant(1, x), mu0, pi0, this->beta, this->h).coeff(0);
	}
};
