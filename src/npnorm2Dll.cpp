...
public:
	...
	mutable Eigen::Matrix<Type, Eigen::Dynamic, 1> dens;
	...
	npnorm2Dll(const Eigen::Matrix<Type, Eigen::Dynamic, 2> &data_, const Eigen::Matrix<Type, Eigen::Dynamic, 2> &mu0fixed_, 
		const Eigen::Matrix<Type, Eigen::Dynamic, 1> &pi0fixed_, const Eigen::Matrix<Type, 2, 2> &beta_, const Eigen::Matrix<Type, Eigen::Dynamic, 2> &initpt_,
		const Eigen::Matrix<Type, Eigen::Dynamic, 1> &initpr_, const Eigen::Matrix<Type, Eigen::Dynamic, 2> &gridpoints_) : data(data_),
	...
	void setdens(const Eigen::Matrix<Type, Eigen::Dynamic, 1> & dens) const{
	...
	Eigen::Matrix<Type, Eigen::Dynamic, 1> mapping(const Eigen::Matrix<Type, Eigen::Dynamic, 2> &mu0, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &pi0) const{
		return dnpnormND_(this->data, mu0, pi0, this->beta);
	}

	void gradfun(const Eigen::Matrix<Type, 1, 2> &mu, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &dens, 
		...
		Eigen::Matrix<Type, Eigen::Dynamic, 1> temp = dnpnormND_(this->data, mu, scale, this->beta);
		if (d0)
	...
	void gradfunvec(const Eigen::Matrix<Type, Eigen::Dynamic, 2> &mu, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &dens,
		Eigen::Matrix<Type, Eigen::Dynamic, 1> &ansd0, Eigen::Matrix<Type, Eigen::Dynamic, 2> &ansd1,
	...
	void checklossfun2(const Eigen::Matrix<Type, Eigen::Dynamic, 1> &diff, Eigen::Matrix<Type, Eigen::Dynamic, 1> &pi0, 
	...
	// collapsing
	void collapse(Eigen::Matrix<Type, Eigen::Dynamic, 2> & mu0, Eigen::Matrix<Type, Eigen::Dynamic, 1> & pi0, const Type &tol = 1e-6) const{
		...
		Eigen::Matrix<Type, Eigen::Dynamic, 1> pi0new = pi0;
		do {
			if (mu0.rows() <= 1) break;
	...
	void computeweights(const Eigen::Matrix<Type, Eigen::Dynamic, 2> &mu0, Eigen::Matrix<Type, Eigen::Dynamic, 1> &pi0, const Eigen::Matrix<Type, Eigen::Dynamic, 1> &dens) const{
	...
	Type operator()(const Eigen::Matrix<Type, Eigen::Dynamic, 1>& x, Eigen::Matrix<Type, Eigen::Dynamic, 1>& grad) const{
		Type ansd0;
	...
	Eigen::Matrix<Type, Eigen::Dynamic, 2> solvegrad(const Eigen::Matrix<Type, Eigen::Dynamic, 1> &dens, const Type &tol = 1e-6) const{
		Eigen::Matrix<Type, Eigen::Dynamic, 2> ans(0, 2); // pointsgrad not referenced.
		Type fval;
		Eigen::Matrix<Type, Eigen::Dynamic, 1> xval(2, 1), lb(2, 1), ub(2, 1);
	...
	void computemixdist(const Type &tol = 1e-6, const int &maxit = 100, const int &verbose = 0) const{
		...
		Eigen::Matrix<Type, Eigen::Dynamic, 1> pi0 = initpr * (1. - pi0fixed.sum());
		// int iter = 0, convergence = 0;
		this->iter = 0;
		Eigen::Matrix<Type, Eigen::Dynamic, 2> newpoints;
		Eigen::Matrix<Type, Eigen::Dynamic, 1> dens = this->mapping(mu0, pi0);
		...
		do{
			...
			pi0.tail(newpoints.rows()) = Eigen::Matrix<Type, Eigen::Dynamic, 1>::Zero(newpoints.rows());
			...
			if (verbose >= 2){
				...
				Eigen::Matrix<Type, Eigen::Dynamic, 1> pointsval;
	...
	Rcpp::List get_ans() const{
		...
		Eigen::Matrix<Type, Eigen::Dynamic, 1> maxgrad;
